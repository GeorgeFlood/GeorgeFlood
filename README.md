...
<p align="center">
  <a href="https://georgeflood.co.uk/">Portfolio</a> ·
  <a href="https://www.linkedin.com/in/george-flood-frontend/">LinkedIn</a> ·
  <a href="mailto:mrgeorgeflood@gmail.com">Email</a>
</p>
...
**[MeetHere](https://meethere.live/)**

A Laravel app that finds a fair meeting point across London for a group. It uses the TfL Unified API to compare journey times and suggest locations that don't quietly stitch one person up with a horrible commute.

`Laravel` `PHP` `TfL API` `Travel data` `MySQL`

**[PostcodeFindr](https://www.postcodefindr.uk/)**

A UK postcode lookup project built on the OS Places API. I shipped it as an API, a web app, and a CLI because I wanted to understand the same product from different angles: backend logic, user interface, and developer tooling.

`Laravel` `PHP` `OS Places API` `CLI` `API design`

**[Portfolio](https://georgeflood.co.uk/)**

A Three.js portfolio built as a single-page scroll experience. It starts with a warp-speed starfield, moves towards a NASA-textured Earth, and drops through the atmosphere into the main site. Mostly built because I wanted my portfolio to feel a bit more alive than another grid of cards.
...
**Backend** &nbsp; `Laravel` `PHP` `MySQL` `REST APIs` `AWS` `Nginx`

**Frontend** &nbsp; `React` `TypeScript` `JavaScript` `Tailwind CSS` `SCSS` `Three.js`

**Tools** &nbsp; `Git` `GitHub` `VS Code` `Cursor` `TablePlus` `Postman`
